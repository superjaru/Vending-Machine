...
jest.mock('../../src/modules/products/products.repo', () => ({
...
jest.mock('../../src/modules/cashFloat/cashFloat.repo', () => ({
...
jest.mock('../../src/modules/transactions/transactions.repo', () => ({
  transactionRepo: {
    create:         jest.fn(),
...
import { productRepo }                     from '../../src/modules/products/products.repo'
import { inventoryRepo }                   from '../../src/modules/inventory/inventory.repo'
import { denominationRepo, cashFloatRepo } from '../../src/modules/cashFloat/cashFloat.repo'
import { transactionRepo }                 from '../../src/modules/transactions/transactions.repo'
...
const createdTx = {
  id: 99, machine_id: 1, product_id: 1, status: 'completed',
  product_price_thb: '20.00', amount_inserted_thb: '100.00', change_given_thb: '80.00',
}
const validDto = {
  machine_id: 1,
  product_id: 1,
...
describe('TransactionService.purchase', () => {
  ...
  it('inserts payment rows with correct transaction id', async () => {
    ...
    expect(transactionRepo.insertPayments).toHaveBeenCalledWith(
      99, validDto.payments, expect.anything(),
    )
  })
  ...
  it('throws 404 when product is inactive or not found', async () => {
    ...
    await expect(service.purchase(validDto))
      .rejects.toThrow(new AppError('Product not found or inactive', 404))
  })

  it('throws 409 when product is out of stock', async () => {
    ;(inventoryRepo.findOne as jest.Mock).mockResolvedValue({ ...inv, stock: 0 })
    await expect(service.purchase(validDto))
      .rejects.toThrow(new AppError('Product out of stock', 409))
  })
  ...
  it('throws 422 when payment is insufficient', async () => {
    ...
    await expect(service.purchase(cheapDto))
      .rejects.toThrow(new AppError('Insufficient payment', 422))
  })
...
describe('TransactionService.getById', () => {
  ...
  beforeEach(() => { service = new TransactionService() })
